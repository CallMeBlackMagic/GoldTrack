...
def get_price_changes_daily(items_list):
    if len(items_list) > 1:
        if items_list[-1] > items_list[-2]:
            ...
            return "Same prices as yesterday"
    else:
...
def get_price_changes_monthly(items_list):
    if len(items_list) > 1:
        if items_list[-1] > items_list[0]:
            ...
            return "Same prices as last month"
    else:
        return 'NOT ENOUGH DATA'
...
def create_daily_report():
    ...
    Coin Ounce price - {price_list[7]}
    Gold Gold price - {price_list[8]}
    -----------------Price Changes------------------------
...
def plot_weekly_plot():
    if len(dates) >= 7:
        gold_plotter = GoldPricePlotter(dates[-7:], column_Global_ounce[-7:], column_24k[-7:], column_22k[-7:], column_21k[-7:], column_18k[-7:], column_14k[-7:], column_12k[-7:], column_ounce[-7:], column_coin[-7:],constants.weekly_plots_path)
        gold_plotter.plot_gold_prices()
    else:
